...
# stage1:
# We depict a top view of a city, simple squares with streets between them.
#
# Streets layout is a simple grid
#
# No scroll needed
#
# Cars spawn out of view, over an empty street crossing.
# At spawn time a destination is selected, which must be another crossing out
# of view. Use some heuristics so that the travel is not too short.
#
# Most of the time multiple cars are in view
#
# After a car reachs its destination it should be re-spawned
#
# Car actors will be implemented as Sprite subclasses with an explicit
# .update(self, dt) method
#
# no collision, that would be next stage
...
##x0, yo : coords street crossing bottom left, out of view
##
##ix,iy : street crossing 0,0 is bottom left (out of view)
##
##ix, iy maps to x,y = ix*crossing_point_separation + iy*crossing_point_separation 

class RobotCar(cocos.sprite.Sprite):
    """
    use create to obtain a proper instance; __init__ will not define .traveling
    """
    @classmethod
    def create(cls):
        car = cls("fire.png")
    ...
    def update_when_crossing_reached(self):
        ...
        self.position = (ix*crossing_point_separation, iy*crossing_point_separation)
